...
function extractSetup(event: any): Setup {
  return {
    style: event.style || null, rig: event.rig || null, line_type: event.line_type || null,
    retrieve: event.retrieve || null, spot: event.spot || null,
    fly_pattern: event.fly_pattern || null, fly_size: event.fly_size ?? null,
  }
}
...
function extractSetupFromChangeFrom(changeFrom: any): Setup {
  ...
  return {
    style: changeFrom.style || null, rig: changeFrom.rig || null, line_type: changeFrom.line_type || null,
    retrieve: changeFrom.retrieve || null, spot: changeFrom.spot || null,
    fly_pattern: changeFrom.fly_pattern || null, fly_size: changeFrom.fly_size ?? null,
  }
}
...
function segmentWeatherPeriods(
  ...
  for (let i = 1; i < weatherLog.length; i++) {
    ...
    if (breached) {
      periods.push({ startMins: periodStart, endMins: parseHHMM(snap.time), snapshots: currentSnapshots, weatherConfidence: 'measured' })
  ...
  periods.push({ startMins: periodStart, endMins, snapshots: currentSnapshots, weatherConfidence: 'measured' })
  ...
  const weatherPeriods = periodBoundaries.map((period) => {
    ...
    for (const event of periodEvents) {
      ...
      if (currentSetup && elapsed > 0) {
        ...
        if (!techniques[tKey]) techniques[tKey] = { technique: currentSetup.style || 'Unknown', rig: currentSetup.rig || '', line: currentSetup.line_type || '', retrieve: currentSetup.retrieve || '', minutes: 0, catches: 0 }
        ...
        if (!flies[fKey]) flies[fKey] = { fly: currentSetup.fly_pattern || 'Unknown', hook_size: currentSetup.fly_size, minutes: 0, catches: 0 }
      ...
      if (event.event_type === 'catch') {
        ...
        if (currentSetup) {
          ...
          else techniques[tKey] = { technique: currentSetup.style || 'Unknown', rig: currentSetup.rig || '', line: currentSetup.line_type || '', retrieve: currentSetup.retrieve || '', minutes: 0, catches: 1 }
      ...
      if (event.event_type === 'change') {
        ...
        allSetupChanges.push({ time: event.event_time, from_technique: oldStyle, to_technique: currentSetup.style || 'Unknown' })
      }
    ...
    return {
      ...
      techniques: Object.values(techniques).map((t: any) => ({ ...t, minutes: Math.round(t.minutes) })),
      spots: Object.values(spots).map((s: any) => ({ ...s, minutes: Math.round(s.minutes) })),
      flies: Object.values(flies).map((f: any) => ({ ...f, minutes: Math.round(f.minutes) })),
...
async function computeAnglerStatsInline(supabase: any, userId: string, venueId: string) {
  ...
  await supabase
    ...
    .upsert({
      ...
      total_sessions: totalSessions, total_fish: totalFish,
      total_hours: Math.round(totalHours * 10) / 10,
      catch_rate: catchRate, fish_per_hour: fishPerHour,
      technique_stats: techniqueStats, last_session_date: lastSessionDate,
...
async function computeVenueStatsInline(supabase: any, venueId: string) {
  ...
  await supabase
    .from('venue_stats')
    .upsert({
      venue_id: venueId, total_diary_sessions: totalSessions,
      total_anglers: uniqueAnglers, mean_catch_rate: meanCatchRate,
      mean_fish_per_hour: meanFishPerHour, diary_date_range: diaryDateRange,
      updated_at: new Date().toISOString(),
    }, { onConflict: 'venue_id' })
...
async function computeOneSession(
  ...
  if (sessErr || !session) throw new Error(`Session not found: ${sessErr?.message || 'no data'}`)
  ...
  return { totalFish, venueId: venue.id, userId: session.user_id }
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const venueFilter = body.venue_name || null
    ...
    if (dryRun) {
      ...
      for (const s of needCompute) {
        byVenue[s.venue_name] = (byVenue[s.venue_name] || 0) + 1
      }
      ...
      return new Response(
        JSON.stringify({
          ...
          needs_compute: needCompute.length,
          by_venue: byVenue,
        }),
    ...
    for (const sess of batch) {
      try {
        ...
        errors.push({ session_id: sess.id, error_message: err.message || String(err) })
      }
    }
